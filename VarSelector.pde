class VarSelector {
  ...
  VarSelector(){
    ...
    for(int i=0;i< 16;i++){
      ...
      buttons[i]=new Button(variable[i],variable[i],bPos, 325, 45);
    }
    for(int i = 0; i < 16; i++) {
      PVector xBPos = new PVector(pos.x + 345, i*50+225);
      PVector yBPos = new PVector(pos.y + 395, i*50+225);
      selectableX[i] = new Button("X", variable[i], xBPos, 50, 45);
      selectableX[i] = new Button("Y", variable[i], yBPos, 50, 45); 
    }
  }
